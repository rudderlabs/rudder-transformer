const { formatBatchResponse, createResponseBodyJSON } = require('./utils');
...
describe('braze utils test cases', () => {
  describe('createResponseBodyJSON util tests', () => {
    it('flow check', () => {
      let expectedOutput = {
        ...
        partner: 'RudderStack',
        purchases: [{ currency: 'USD', revenue: 10.9 }],
      };
      expect(createResponseBodyJSON(attributesBatch, eventsBatch, purchaseBatch)).toEqual(
        expectedOutput,
      );
    });
  });
  ...
  describe('formatBatchResponse util tests', () => {
    it('flow check', () => {
      let expectedOutput = {
        batchedRequest: { dummyKey: 'dummyValue' },
        destination: { ID: 'a' },
        metadata: [{ job_id: 1 }],
      };
      expect(formatBatchResponse({ dummyKey: 'dummyValue' }, [{ job_id: 1 }], { ID: 'a' })).toEqual(
        expectedOutput,
      );
    });
  });
