...
const buildPageLoadPayload = (message, campaignId, impactAppId, enableEmailHashing) => {
  ...
  if (isDefinedAndNotNull(payload.CustomerEmail) && typeof payload?.CustomerEmail === 'string') {
...
const processCommonEvents = (message, Config) => {
  ...
  if (message.type === 'identify' && !enableIdentifyEvents) {
    throw new ConfigurationError(`${message.type} events are disabled from Config`);
  }
...
const processTrackEvent = (message, Config) => {
  ...
  if (!eventType.includes('install') && message.event === 'Applcation Installed') {
    eventType.push('install');
  } else if (!eventType.includes('action') && message.event === 'Order Completed') {
    eventType.push('action');
  }

  if (eventType.length > 0) {
    let payload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.CONVERSION.name]);
    payload.CampaignId = campaignId;
    ...
    const productProperties = populateProductProperties(productsMapping, message.properties);
