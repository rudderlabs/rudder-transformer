...
const responseBuilderSimple = (payload, category, destination) => {
  if (payload) {
    ...
    switch (category.name) {
      case "ACIdentify":
      case "ACPage":
        response.endpoint = `${destination.Config.apiUrl}${category.endPoint ? category.endPoint : ""
          }`;
...
const customTagProcessor = async (message, category, destination) => {
  ...
  try {
    res = await axios.post(
      `${destination.Config.apiUrl}${category.endPoint ? category.endPoint : ""
      }`,
      {
  ...
  try {
    res = await axios.get(
      `${destination.Config.apiUrl}${category.tagEndPoint ? `${category.tagEndPoint}?limit=100` : ""
      }`,
      {
        headers: {
          "Content-Type": "application/json",
  ...
  if (res.status === 200) {
    ...
    if (res.data.meta && res.data.meta.total && parseInt(res.data.meta.total) > 100) {
      for (i = 0; i < Math.floor(parseInt(res.data.meta.total) / 100); i++) {
        try {
          res = axios.get(
            `${destination.Config.apiUrl}${category.tagEndPoint ? `${category.tagEndPoint}?limit=100&offset=${100 * (i + 1)}` : ""
            }`,
            {
              headers: {
                "Content-Type": "application/json",
    ...
    if (tagsToBeCreated.length > 0) {
      await Promise.all(
        tagsToBeCreated.map(async tag => {
          try {
            res = await axios.post(
              `${destination.Config.apiUrl}${category.tagEndPoint ? category.tagEndPoint : ""
              }`,
              {
                tag: {
                  tag,
    ...
    await Promise.all(
      tagIds.map(async tagId => {
        try {
          res = await axios.post(
            `${destination.Config.apiUrl}${category.mergeTagWithContactUrl
  ...
  const customFieldProcessor = async (
    ...
    try {
      res = await axios.get(
        `${destination.Config.apiUrl}${category.fieldEndPoint ? category.fieldEndPoint : ""
        }`,
        {
          headers: {
            "Api-Token": destination.Config.apiKey
    ...
    await Promise.all(
      filteredFieldKeys.map(async key => {
        ...
        try {
          await axios.post(
            `${destination.Config.apiUrl}${category.mergeFieldValueWithContactUrl
    ...
    Promise.all(
      listArr.map(async li => {
        if (li.status === "subscribe" || li.status === "unsubscribe") {
          try {
            await axios.post(
              `${destination.Config.apiUrl}${category.mergeListWithContactUrl
  ...
  const screenRequestHandler = async (message, category, destination) => {
    ...
    try {
      res = await axios.get(
        `${destination.Config.apiUrl}${category.getEventEndPoint ? category.getEventEndPoint : ""
        }`,
        {
          headers: {
            "Content-Type": "application/json",
