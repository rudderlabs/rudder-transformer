...
- **User Identifier Source**: Specifies the source of user identifiers
  - Options: `UNSPECIFIED`, `UNKNOWN`, `FIRST_PARTY`, `THIRD_PARTY`
- **Conversion Environment**: Specifies the conversion environment
  - Options: `UNSPECIFIED`, `UNKNOWN`, `APP`, `WEB`
...
The Google Ads API enforces rate limits using a Token Bucket algorithm. Here are the rate limits for the endpoints used by this destination:

| Endpoint | Conversion Types | Rate Limit | Batch Limits | Description |
|----------|------------------|------------|--------------|-------------|
...
Rate limit violations will result in `RESOURCE_TEMPORARILY_EXHAUSTED` errors. The Google Ads API uses QPS (Queries Per Second) rate limiting per client customer ID and developer token.
...
Google Ads API maintains multiple versions simultaneously. Check the [Google Ads API versioning documentation](https://developers.google.com/google-ads/api/docs/concepts/versioning) for the latest version information and migration guides.
...
- [Click Conversions Upload](https://developers.google.com/google-ads/api/rest/reference/rest/v19/customers/uploadClickConversions)
- [Call Conversions Upload](https://developers.google.com/google-ads/api/rest/reference/rest/v19/customers/uploadCallConversions)
