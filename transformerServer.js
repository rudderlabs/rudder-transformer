        ...
        if (request.method == "POST" && pathname != "/reload") {
          ...
          request.on("end", async function() {
            try {
              ...
              var respList;
              respList = route(pathname, request, response, body)
              ...
              switch (se.constructor.name) {
                case "RangeError":
                  response.statusCode = 400; // 400 for unexpected value as well
                ...
                case "SyntaxError":
                  ...
                  response.statusMessage =
                    "Malformed JSON payload " + se.message;
                  break;
                default:
                  response.statusCode = 500; // 500 for other errors
                  response.statusMessage = se.message;
          ...
          delete require.cache[
            require.resolve("./" + query.version + "/" + query.name + ".js")
          ];
          response.statusCode = 200;
          response.end();
        }
