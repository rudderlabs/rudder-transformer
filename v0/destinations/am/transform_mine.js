...
function processRevenue(message, destination) {
  const toSendEvents = [];
  const simpleTrackEvent = JSON.parse(JSON.stringify(message));
  const revenueEvent = JSON.parse(JSON.stringify(message));
  ...
  for (const key of Object.keys(message.properties)) {
    if (
      ...
      if (
        (key === "products" &&
          destination.Config.trackProductOnce === "false") ||
        key !== "products"
      ) {
        delete simpleTrackEvent.properties[key];
      }
    } else {
      delete revenueEvent.properties[key];
    }
  }
  if (revenueEvent.properties.products) {
    delete revenueEvent.properties.products;
  }
  toSendEvents.push(simpleTrackEvent);
  if (
    message.properties.products &&
    destination.Config.trackProductOnce === "false"
  ) {
    message.properties.products.forEach(product => {
      const productPurchasedEvent = { ...message };
      productPurchasedEvent.properties = product;
      productPurchasedEvent.event = "Product Purchased";
      if (destination.Config.trackRevenuePerProduct === "true") {
        const eachproductRevenue = JSON.parse(
          JSON.stringify(productPurchasedEvent)
        );
        if (eachproductRevenue.properties.revenue) {
          // eslint-disable-next-line no-param-reassign
          eachproductRevenue.properties.price = product.revenue;
          // eslint-disable-next-line no-param-reassign
          eachproductRevenue.properties.quantity = 1;
          // eslint-disable-next-line no-param-reassign
          delete eachproductRevenue.properties.revenue;
        }
        // eslint-disable-next-line no-restricted-syntax
        for (const key of Object.keys(productPurchasedEvent.properties)) {
          if (key !== "revenue" && key !== "price" && key !== "quantity") {
            delete eachproductRevenue.properties[key];
          } else {
            delete productPurchasedEvent.properties[key];
          }
        }
        toSendEvents.push(productPurchasedEvent);
        toSendEvents.push(eachproductRevenue);
      } else {
        delete productPurchasedEvent.properties.revenue;
        delete productPurchasedEvent.properties.price;
        delete productPurchasedEvent.properties.quantity;
        toSendEvents.push(productPurchasedEvent);
      }
    });
  }
  if (
    ...
    if (revenueEvent.properties.price) {
      delete revenueEvent.properties.price;
    }
    revenueEvent.properties.quantity = 1;
    revenueEvent.properties.price = revenueEvent.properties.revenue;
    delete revenueEvent.properties.revenue;
...
function process(event) {
  ...
  if (messageType === EventType.TRACK) {
    if (message.properties && message.properties.revenue) {
      const tempResult = processRevenue(message, destination);
