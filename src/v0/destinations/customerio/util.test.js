const {isdeviceRelatedEventName, identifyResponseBuilder, aliasResponseBuilder, groupResponseBuilder} = require('./util');
...
describe('Unit test cases for customerio identifyResponseBuilder', () => {
    it('Device token name does not match with event name as well as allowed list', async () => {
        expectedOutput = {"endpoint": "https://track.customer.io/api/v1/customers/user1", "rawPayload": {"anonymous_id": "anonId", "city": "kolkata", "country": "India", "created_at": 1400687660, "email": "abc@test.com", "name": "rudder", "timestamp": "2014-05-21T15:54:20Z"}, "requestConfig": {"requestFormat": "JSON", "requestMethod": "PUT"}}
...
describe('Unit test cases for customerio aliasResponseBuilder', () => {
    it('Device token name does not match with event name as well as allowed list', async () => {
        expectedOutput = {"endpoint": "https://track.customer.io/api/v1/merge_customers", "rawPayload": {"primary": {"id": "user1"}, "secondary": {"id": undefined}}, "requestConfig": {"requestFormat": "JSON", "requestMethod": "POST"}}
    ...
    describe('Unit test cases for customerio groupResponseBuilder', () => {
        it('Device token name does not match with event name as well as allowed list', async () => {
            expectedOutput = {"endpoint": "https://track.customer.io/api/v2/batch", "rawPayload": {"action": "identify", "attributes": {"address": {"city": "kolkata", "country": "India"}, "createdAt": "2014-05-21T15:54:20Z", "email": "abc@test.com", "name": "rudder", "objectTypeId": "obj1", "timestamp": "2014-05-21T15:54:20Z"}, "cio_relationships": [], "identifiers": {"object_id": "group1", "object_type_id": "obj1"}, "type": "object"}, "requestConfig": {"requestFormat": "JSON", "requestMethod": "POST"}}
