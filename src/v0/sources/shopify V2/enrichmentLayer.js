...
const enrichmentLayer = {
     ...
     * @returns 
     */
    getAnonymousIdAndSessionId(message, metricMetadata, redisData = null) {
        let anonymousId;
        let sessionId;
    ...
    enrichMessage(event, message, redisData, metricMetadata) {
        ...
        if (message.type !== EventType.IDENTIFY) {
            const { anonymousId, sessionId } = await this.getAnonymousIdAndSessionId(message, metricMetadata, redisData);
            if (isDefinedAndNotNull(anonymousId)) {
        ...
        updatedMessage.setProperty(`integrations.${INTEGERATION}`, true);
        updatedMessage.setProperty('context.library', {
            name: 'RudderStack Shopify Cloud',
            version: '1.0.0',
        });
        message.setProperty('context.topic', shopifyTopic);
        // attaching cart, checkout and order tokens in context object
        updatedMessage.setProperty(`context.cart_token`, event.cart_token);
        updatedMessage.setProperty(`context.checkout_token`, event.checkout_token);
        if (shopifyTopic === 'orders_updated') {
            updatedMessage.setProperty(`context.order_token`, event.token);
        }
        return updatedMessage;
    }
}
module.exports = { enrichmentLayer };