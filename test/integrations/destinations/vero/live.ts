...
// Vero's Track API is WRITE-ONLY (no read-back endpoint), so every scenario is delivery-only: the
// pipeline step asserts the real API accepted the payload (2xx via /routerTransform -> /proxy).
// Setup steps create prerequisite users so alias/tags/(un)subscribe act on a real record; cleanup
