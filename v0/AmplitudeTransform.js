...
var promotionViewedConfigFile = fs.readFileSync(
  "data/AmplitudePromotionViewedConfig.json"
);
var promotionViewedConfigJson = JSON.parse(promotionViewedConfigFile);

var promotionClickedConfigFile = fs.readFileSync(
  "data/AmplitudePromotionClickedConfig.json"
);
var promotionClickedConfigJson = JSON.parse(promotionClickedConfigFile);

var productActionsConfigFile = fs.readFileSync(
  "data/AmplitudeProductActionsConfig.json"
);
var productActionsConfigJson = JSON.parse(productActionsConfigFile);

var coinsPurchasedConfigFile = fs.readFileSync(
  "data/AmplitudeCoinsPurchasedEventConfig.json"
);
var coinsPurchasedConfigJson = JSON.parse(coinsPurchasedConfigFile);

var revenueGeneratedConfigFile = fs.readFileSync(
  "data/AmplitudeRevenueConfig.json"
);
...
var customerCredentialsConfig = fs.readFileSync(
  "data/AmplitudeCredentialsConfig.json"
);
...
function processSingleMessage(jsonQobj) {
  ...
  switch (messageType) {
    ...
    case "track":
      ...
      switch (
        rlEventType.toLowerCase() // decide config to be used based on RL event
      ) {
  ...
  if (error) {
    // error has occurred, single error messge response
    events.push('{"error":"message type not supported"}');
  } else {
    // proper processing, return payload from processing function
    events = responseBuilderSimple(
      parameterMap,
      payloadObjectName,
      jsonQobj,
      amplitudeEventType,
...
function processTransaction(jsonQobj, respList) {
  ...
  var transactionEvent = String(
    jsonQobj.find("rl_event").value()
  ).toLowerCase();
  if (
    transactionEvent == "order cancelled" ||
...
function process(jsonQobj) {
  ...
  jsonQobj.find("rl_message").each(function(index, path, value) {
    ...
    var messageType = String(
      singleJsonQObj.find("rl_type").value()
    ).toLowerCase();
    var rlEventType = String(
      singleJsonQObj.find("rl_event").value()
    ).toLowerCase();
    // console.log(++counter);
