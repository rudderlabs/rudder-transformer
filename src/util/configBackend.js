...
// Basic-auth request options for the config backend, with the dedicated transformation-fetch secret
// as the username (kept separate from the hosted secret so a leak here cannot read namespace config).
...
function configBackendRequestOptions() {
  const secret = process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET?.trim();
  if (!secret) return {};
