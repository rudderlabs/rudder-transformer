...
describe('config backend fetch e2e (mock enforces auth)', () => {
  ...
  const savedSecret = process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET;
  ...
  afterAll(async () => {
    ...
    if (savedSecret === undefined) delete process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET;
    else process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET = savedSecret;
  });

  it('fetches successfully when the correct secret is sent, proving the header reaches the wire', async () => {
    process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET = SECRET;
  ...
  it('gets a 401 from the mock when the header is missing, surfaced as a retriable 503', async () => {
    delete process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET;
