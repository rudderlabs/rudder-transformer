...
const buildTrackPayload = (event) => {
  ...
  switch (event.event.eventType) {
    ...
    case 'SEGMENT_IO':
      ...
      break;
    default:
      throw new TransformationError(`Event type ${event.event.eventType} not supported`);
  }
...
function processEvent(event) {
  ...
  if (!message?.userId) {
    message.anonymousId = event?.event?.sessionId ? event.event.sessionId : md5(event.user.email);
  }
  return message;
}
