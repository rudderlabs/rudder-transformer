...
describe("User transformation with IVM cache", () => {
  beforeEach(() => {
    jest.resetAllMocks();
  ...
  it(`Simple ${name} async test for V1 transformation - transformEvent returning array of events`, async () => {
    ...
    expect(outputCached).toEqual(matchData);
  });
  ...
  describe("UserTransformation With Credentials for code version 1", () => {
    ...
    it(`throws TypeError if the key provided for credential lookup is null or undefined`, async () => {
      ...
      const outputCached = await userTransformHandler(inputData, versionId, []);
      expect(outputCached[0].error).toMatch(/Key should be valid and defined/);
    });
    ...
    it(`allows user to pass valid key of any type and performs lookup accordingly`, async () => {
      ...
      const outputCached = await userTransformHandler(inputData, versionId, []);
      expect(outputCached[0].transformedEvent.credentialValueForNumkey).toBeUndefined();
      expect(outputCached[0].transformedEvent.credentialValueForBoolkey).toBeUndefined();
      expect(outputCached[0].transformedEvent.credentialValueForArraykey).toBeUndefined();
      expect(outputCached[0].transformedEvent.credentialValueForObjkey).toBeUndefined();
    });
    ...
    it(`returns undefined when the key doesn't match any credential lookup`, async () => {
      ...
      const outputCached = await userTransformHandler(inputData, versionId, []);
      expect(outputCached[0].transformedEvent.credentialValue).toBeUndefined();
    });
    ...
    describe('Batch UserTransformation with Credentials', () => {
      it(`successfully executes transformation with credential lookup with valid key`, async () => {
        ...
        fetch.mockResolvedValue({
          ...
          // Call the same function to use the cached isolate vm
          const outputCached = await userTransformHandler(inputData, versionId, []);
          expect(fetch).toHaveBeenCalledWith(
            `https://api.rudderlabs.com/transformation/getByVersionId?versionId=${versionId}`
          );
          expect(outputCached[0].transformedEvent.credentialValue1).toEqual("value1");
          expect(outputCached[0].transformedEvent.credentialValue2).toBeUndefined();
      });
