...
const handleProductListViewed = (message, categoryToContent) => {
  const {
    ...
    brand,
    variant,
    price: priceString, // Renamed to avoid conflict with modified price
    currency,
    position,
    url,
    image_url: imageUrl,
  ...
  const price = parseFloat(priceString); // Convert price to a number
  ...
  return {
    products,
    // other properties you may include
  };
};
