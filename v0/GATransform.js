...
var promotionEventConfigFile = fs.readFileSync(
  "data/GAPromotionEventConfig.json"
);
var promotionEventConfigJson = JSON.parse(promotionEventConfigFile);

var paymentRelatedEventConfigFile = fs.readFileSync(
  "data/GAPaymentRelatedEventConfig.json"
);
...
var productListEventConfigFile = fs.readFileSync(
  "data/GAProductListEventConfig.json"
);
var productListEventConfigJson = JSON.parse(productListEventConfigFile);

var transactionEventConfigFile = fs.readFileSync(
  "data/GATransactionEventConfig.json"
);
...
var ecomGenericEventConfigFile = fs.readFileSync(
  "data/GAEComGenericEventConfig.json"
);
...
var customerCredentialsConfig = fs.readFileSync(
  "data/GACustomerCredentialsConfig.json"
);
...
function responseBuilderSimple(
  ...
  jsonQobj.find("anonymousId").each(function(index, path, value) {
    responseMap.set("userId", String(value));
  });
...
function processTransactionEvent(jsonQobj) {
  ...
  if (!revenueString || revenueString.length === 0) {
    ...
    if (!valueString || valueString.length === 0) {
      ...
      if (!(!totalString || totalString.length === 0)) {
