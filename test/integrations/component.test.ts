...
const testRoute = async (route, tcData: TestCaseData) => {
  ...
  switch (inputReq.method) {
    case "GET":
      ...
      break;
    case "PUT":
      response = await request(server).put(route)
        .set(headers || {}).query(params || {}).send(body);
      break;
    case "DELETE":
      response = await request(server).delete(route)
        .set(headers || {}).query(params || {}).send(body);
      break;
    
    default:
      response = await request(server).post(route)
        .set(headers || {}).query(params || {}).send(body);
      break;
  }
