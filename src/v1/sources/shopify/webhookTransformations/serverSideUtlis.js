...
const setAnonymousId = async (message, event, metricMetadata) => {
  ...
  if (isDefinedAndNotNull(anonymousId)) {
    ...
    if (cartToken) {
      ...
      const anonymousIdFromCartToken = v5(cartToken, v5.DNS);
      if (redisData?.anonymousId) {
        ...
        // also adding this anonymousId inside traits object in cart_token_hash key
        message.anonymousId = anonymousIdFromCartToken;
      }
      message.traits = { ...message.traits, cart_token_hash: anonymousIdFromCartToken };
      console.log('message after cart_token_hash', message);
      // Object.assign(message, {
      //   traits: {
      //     ...message.traits,
      //     cart_token_hash: anonymousIdFromCartToken,
      //   },
      // });
    } else {
