  ...
  try {
    ...
    for (const file of allFilesInFolder) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const regex = /Authorization['":]?\s*['":]?\s*(Bearer|Token)\s+([^'"]+)['"]/gi;
        let match;
        ...
        while ((match = regex.exec(line)) !== null) {
          ...
          if (!existingMatch.authLocations.some((loc) => loc.file === file && loc.line === i + 1)) {
...
function createUniqueSecrets(matches: SecretMatch[]): MaskedSecret[] {
  ...
  for (const match of matches) {
    ...
    if (!secretMap.has(secretKey)) {
      const secretConstName = `SECRET_${index}`;
    ...
    if (!secretMap.has(authKey)) {
      const authConstName = `AUTH_PATTERN_${index}`;
...
if (require.main === module) {
  const rootDir = process.argv[2] || '../integrations/destinations/';
