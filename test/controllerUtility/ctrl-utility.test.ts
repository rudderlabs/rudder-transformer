...
const timestampEventsCases: timestampTestCases[] = [
  {
    caseName: 'when proc events(all track), timestamp should be taken from properties.timestamp',
  ...
  {
    caseName:
      'when proc events(all track) without properties.timestamp, timestamp should be taken from timestamp',
    expectedOutputEvents: [
      {
        message: {
          anonymousId: '2073230',
  ...
  {
    caseName:
      'when proc events(all identify) without context.timestamp, timestamp should be taken from timestamp',
    expectedOutputEvents: [
      {
        message: {
          anonymousId: '2073230',
