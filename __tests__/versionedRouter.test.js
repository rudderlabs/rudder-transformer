...
// eg: jest versionedRouter --destName=heap --type=process 
...
if (destArg && typeArg) {
  ...
  const reqBody = JSON.parse(fs.readFileSync(
    path.resolve(__dirname, `./data/versioned_${type}_${destination}_input.json`)
  ));
  const respBody = JSON.parse(fs.readFileSync(
  ...
  if (type === "router") {
    ...
    it(`Testing: handleDest`, async () => {
      const output = await vRouter.handleDest(
        reqBody,
        version,
        destination
      );
    ...
    it(`Testing: batchHandler`, async () => {
      const output = await vRouter.batchHandler(
        reqBody
      );
      expect(output).toEqual(respBody);
    });
