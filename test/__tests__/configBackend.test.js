...
describe('configBackendRequestOptions', () => {
  const original = process.env.CONFIG_BACKEND_HOSTED_SECRET;

  afterEach(() => {
    if (original === undefined) {
      delete process.env.CONFIG_BACKEND_HOSTED_SECRET;
    } else {
      process.env.CONFIG_BACKEND_HOSTED_SECRET = original;
    }
  });
  ...
  it('returns empty options (not { headers: {} }) when the secret is blank/whitespace', () => {
    process.env.CONFIG_BACKEND_HOSTED_SECRET = '   ';
    expect(configBackendRequestOptions()).toEqual({});
  });

  it('builds Basic auth with the secret as username and exactly one trailing colon', () => {
    process.env.CONFIG_BACKEND_HOSTED_SECRET = 'my-secret';
    expect(configBackendRequestOptions()).toEqual({
