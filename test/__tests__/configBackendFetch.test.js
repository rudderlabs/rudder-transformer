...
describe('config backend fetch: auth header on the wire and retry on 401/403', () => {
  const originalSecret = process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET;
  ...
  afterEach(() => {
    if (originalSecret === undefined) {
      ...
      process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET = originalSecret;
    }
  });

  it('sends the Authorization header on the transformation fetch when the secret is set', async () => {
    process.env.CONFIG_BACKEND_TRANSFORMATION_FETCH_SECRET = 'sekret';
