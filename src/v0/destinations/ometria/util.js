const { lineitemsMapping, variantMapping, LINEITEMS_EXCLUSION_FIELDS } = require('./config');
  ...
  if (items.length > 0) {
    items.forEach((item, index) => {
      ...
      if (
        ...
        if (itemPayload.is_on_sale && typeof itemPayload.is_on_sale !== 'boolean') {
...
const addressMappper = (address) => {
  ...
  const res = {
    ...
    country_code: getValueFromMessage(address, ['country_code', 'country', 'countryCode']),
    postcode: getValueFromMessage(address, ['postcode', 'postalCode', 'postalcode']),
  };
  return res;
};
