...
// Basic-auth request options for the config backend, with the dedicated transformer service secret
...
function configBackendRequestOptions() {
  const secret = process.env.CONFIG_BACKEND_TRANSFORMER_SERVICE_SECRET?.trim();
  if (!secret) return {};
