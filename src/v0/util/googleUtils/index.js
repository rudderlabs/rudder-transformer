...
const populateConsentFromConfig = (config, consentConfigMap) => {
  ...
  if (
    consentConfigMap.hasOwnProperty('userDataConsent') &&
    consentConfigMap.hasOwnProperty('personalizationConsent')
  ) {
  ...
  if (
    consentConfigMap.hasOwnProperty('analyticsUserDataConsent') &&
    consentConfigMap.hasOwnProperty('analyticsPersonalizationConsent')
  ) {
    if (config?.userDataConsent && GA4_ALLOWED_CONSENT_STATUS.includes(config.userDataConsent)) {
      consent.ad_user_data = config.userDataConsent;
    }

    if (
      config?.personalizationConsent &&
      GA4_ALLOWED_CONSENT_STATUS.includes(config.personalizationConsent)
    ) {
      consent.ad_personalization = config.personalizationConsent;
    }
  }
