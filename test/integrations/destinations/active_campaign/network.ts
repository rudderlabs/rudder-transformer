...
export const networkCallsData = [
  ...
  // --- Track scenario: GET eventTrackingEvents fails with a 502 Bad Gateway ---
  // Keyed on a unique Api-Token so it never collides with the JSON eventTrackingEvents mocks above.
  {
    httpReq: {
      headers: {
        'Api-Token': 'htmlLoginApiToken',
      },
    ...
    httpRes: {
      data: {
        ...
        zone: 'dummy-account.example.com',
        cloudflare_error: true,
        retryable: true,
        retry_after: 60,
        owner_action_required: true,
        what_you_should_do:
          '**Wait and retry.** Back off for at least 60 seconds. If the error persists, the website operator should check their origin server health and configuration.',
        footer: 'This error was generated by Cloudflare on behalf of the website owner.',
      },
      status: 502,
    },
  },
  // --- Invalid/expired API key: the account serves a 200 HTML login shell for an unauthenticated
  //     request, on BOTH the list (GET) and create (POST) event calls. ---
  {
    httpReq: {
      headers: {
        'Api-Token': 'invalidCredsApiToken',
      },
    ...
    httpRes: { data: '<html>login</html>', status: 200 },
  },
  {
    httpReq: {
      data: { eventTrackingEvent: { name: 'Tracking Action' } },
